...
`dehub` has extensive configuration options.

- Modify the default config which is usually under `$HOME/.config/dehub/config.yml`.
- Use the [`--config`][01] flag to specify a different configuration file.
- Create a `.dehub.yml` file in a repo for project specific configuration.
...
The configuration for `dehub` is schematized. The pages in this section list the configuration
...
<CardGrid>
  ...
  <LinkCard
    ...
    description="Documentation for configuring GitHub Actions sections."
  />
  <LinkCard
    title="Keybindings"
    href="./keybindings"
