...
- Only fetch 20 PRs, issues, notifications, and Actions workflow runs at a time for each section.
...
the width is `6`, `gh-dehub` displays as `gh-d`.
...
### Actions Fetch Limit (`actionsLimit`)

| Type    | Minimum | Default |
| :------ | :-----: | :-----: |
| Integer |    1    |   20    |

This setting defines how many workflow runs the dashboard should fetch for each Actions section when:
...
[toggle preview pane]: /getting-started/keybindings/preview/#p---cycle-preview-pane
...
This setting specifies whether the user needs to confirm when quitting `dehub`
When this is on, `dehub` shows a prompt that requires the user to press <kbd>y</kbd>/<kbd>Enter</kbd> to actually quit.
Pressing any other key dismisses the message.

By default, dehub doesn't need a confirmation.
