...
The second key defines an exact mapping between the `dlvhdr/gh-dehub` repository and the
`~/code/dehub` folder. Because this is a non-wildcard mapping, the dashboard will use
this value to resolve the repository path for `dlvhdr/gh-dehub` even though there's also
an entry for `dlvhdr/*`.
...
a wildcard. If a key ends with a wildcard but the value doesn't, `dehub` won't be
able to correctly map repositories to folders.

### Without a Wildcard

If the key for an `repoPath` entry doesn't have a wildcard (`*`), its value must not
have a wildcard. If a key ends without a wildcard but the value does, `dehub` won't
