...
In addition to GitHub's filters, dehub adds templating functions.
...
By default, if the directory you launch `dehub` from is a clone of a remote GitHub repo (or if you
...
have an explicit `repo:` field, `dehub` adds a `repo:<RepoName>` field to the search-bar value for
them (where _`<RepoName>`_ is the name of the remote repo).

That is, `dehub` further filters those sections down to only the PRs/issues for the GitHub
repo name specified in your `GH_REPO` environment variable — or else the repo name of the remote
tracked by the clone directory from which `dehub` launched.

For that, `dehub` first checks and uses the repo name in the `GH_REPO` environment variable (if
you have that set). If `dehub` doesn’t find that, then it next checks for the value of the remote
repo name tracked by the clone directory from which you launched `dehub` — by looking through all
...
…and, otherwise, if `dehub` finds no remotes with any of those names, then it uses the repo name
