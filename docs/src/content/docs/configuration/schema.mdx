...
The `dehub` configuration schema is published here:

[`https://dehub.dev/schema.json`][02]
...
3. Add the following line to the top of your configuration file:
   ...
   ```
   # yaml-language-server: $schema=https://dehub.dev/schema.json
   ```
...
1. Open your `dehub` configuration file.
...
1. Instead of adding a comment to your configuration file, you could create the
   `.vscode/settings.json` file in your `dehub` configuration folder and add this setting:

   ```json
   {
     "yaml.schemas": {
       "https://dehub.dev/schema.json": "*.yml"
     }
   }
   ```
